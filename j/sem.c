...
int main(void) {
	...
	while (choice != 'q') {
		...
		case 'n':
			...
			printf("Enter other permission (0-7): ");
			int other;
			scanf("%d", &other);

			printf("Calling sem_open('%s', O_CREAT | O_EXCL, %o)\n", sem_name, 0600 + group * 8 + other);
			semdes = sem_open(sem_name, O_CREAT | O_EXCL, 0600 + group * 8 + other, value);
